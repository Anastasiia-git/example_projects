...
public class SimpleBoard implements Viewable {
    ...
    public Status makeMove(Move move) {
		...
		System.out.println("Entity is not on the board " + move.toString());
            }
            else if (ent.getColor() != turn) {
                debug.send(LEVEL_1, BOARD, "Wrong color: entity color = " + ent.getColor() + ", move = " + move.toString());
		System.out.println("Wrong color: entity color = " + ent.getColor() + ", move = " + move.toString());
            }
            else {debug.send(LEVEL_1, BOARD, "Move does not exist: turn = " + turn + ", move = " + move.toString());
		System.out.println("Move does not exist: turn = " + turn + ", move = " + move.toString());
		}
            return status;
        }
    ...
    /**
     * Returns the representation of the board as a 2-dimensional array of Entities.
     *
     ...
     */
    protected Vector<Position> findPositionsInRange(Position center, int radius) {
